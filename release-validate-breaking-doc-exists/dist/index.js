...
(() => {
  ...
  const promises_namespaceObject = __WEBPACK_EXTERNAL_createRequire(
    import.meta.url
  )("fs/promises"); // CONCATENATED MODULE: external "node:http"
   ...
   * @returns {boolean}
   */
  function checkForBreakingChanges(inputs) {
  ...
  async function checkFileExists(filePath) {
    ...
    try {
      await promises_namespaceObject.access(
        filePath,
        promises_namespaceObject.constants.F_OK
      );
      return true;
    } catch (err) {
      return false;
   ...
   * @returns {Promise<void>}
   */
  async function publishCommentOnPR(commentContent, inputs) {
   ...
   * @returns {Promise<void>}
   */
  async function deleteComments(commentBodyInclude, inputs) {
    try {
  ...
  async function run() {
    try {
      ...
      const isBreakingChange = checkForBreakingChanges(inputs);
      if (!isBreakingChange) {
      ...
      await deleteComments(matchCommentString, inputs);
  ...
  async function validateBreakingChangeDocExists(breakingChangeDocFilePath) {
    return await checkFileExists(breakingChangeDocFilePath);
  }
  ...
  async function doPost(inputs, contentFilePath, isCodeBlock) {
    let content = await readFile(contentFilePath);
    ...
    await publishCommentOnPR(content, inputs);
  }
