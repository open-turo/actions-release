...
(() => {
  ...
  var external_fs_ = __nccwpck_require__(7147); // CONCATENATED MODULE: external "node:http"
  ...
  const external_node_zlib_namespaceObject = __WEBPACK_EXTERNAL_createRequire(
    import.meta.url
  )("node:zlib"); // CONCATENATED MODULE: external "node:stream"
  const external_node_stream_namespaceObject = __WEBPACK_EXTERNAL_createRequire(
    import.meta.url
  )("node:stream"); // CONCATENATED MODULE: external "node:buffer"
  ...
  /* harmony default export */ const dist = dataUriToBuffer; // CONCATENATED MODULE: external "node:util"
  //# sourceMappingURL=index.js.map
  ...
  const pipeline = (0, external_node_util_namespaceObject.promisify)(
    external_node_stream_namespaceObject.pipeline
  );
  ...
  class Body {
    constructor(body, { size = 0 } = {}) {
      ...
      if (body === null) {
        ...
        external_node_util_namespaceObject.types.isAnyArrayBuffer(body)
      ) {
        // Body is ArrayBuffer
      ...
      if (external_node_buffer_namespaceObject.Buffer.isBuffer(body)) {
        ...
        stream = external_node_stream_namespaceObject.Readable.from(
          body.stream()
        );
      }
  ...
  Body.prototype.buffer = (0, external_node_util_namespaceObject.deprecate)(
  ...
  Object.defineProperties(Body.prototype, {
    ...
    data: {
      get: (0, external_node_util_namespaceObject.deprecate)(
        () => {},
  ...
  const getNonSpecFormDataBoundary = (0,
  external_node_util_namespaceObject.deprecate)(
  ...
  class Headers extends URLSearchParams {
    ...
    constructor(init) {
      ...
      if (init instanceof Headers) {
        ...
        typeof init === "object" &&
        !external_node_util_namespaceObject.types.isBoxedPrimitive(init)
      ) {
        ...
        if (method == null) {
          ...
          result = [...init]
            .map((pair) => {
              if (
                typeof pair !== "object" ||
                external_node_util_namespaceObject.types.isBoxedPrimitive(pair)
              ) {
                throw new TypeError(
  ...
  async function fetch(url, options_) {
    return new Promise((resolve, reject) => {
      ...
      const abort = () => {
        ...
        if (
          request.body &&
          request.body instanceof external_node_stream_namespaceObject.Readable
        ) {
      ...
      request_.on("response", (response_) => {
            ...
            case "follow": {
              ...
              if (
                ...
                options_.body instanceof
                  external_node_stream_namespaceObject.Readable
              ) {
                reject(
                  new FetchError(
        ...
        let body = (0, external_node_stream_namespaceObject.pipeline)(
          response_,
          new external_node_stream_namespaceObject.PassThrough(),
          (error) => {
            if (error) {
              reject(error);
            }
        ...
        if (codings === "gzip" || codings === "x-gzip") {
          body = (0, external_node_stream_namespaceObject.pipeline)(
            body,
        ...
        if (codings === "deflate" || codings === "x-deflate") {
          ...
          const raw = (0, external_node_stream_namespaceObject.pipeline)(
            response_,
            new external_node_stream_namespaceObject.PassThrough(),
            (error) => {
              if (error) {
  ...
  var core = __nccwpck_require__(2186); // CONCATENATED MODULE: ./utils.js
  /**
   * Checks if the next version is a breaking change
   * @param current_version
   * @param next_version
   */
  function checkForBreakingChanges(current_version, next_version) {
    core.info("Checking for breaking changes");
    const actual = Number(current_version) + 1;
    const expected = Number(next_version);
  ...
  async function checkFileExists(filePath) {
    core.info("Checking if file exists: " + filePath);
    try {
      await external_fs_.promises.access(
        filePath,
        external_fs_.promises.constants.F_OK
      );
      return true;
    } catch (err) {
      return false;
  ...
  function readFile(filePath) {
    ...
    try {
      return external_fs_.promises.readFile(filePath, "utf8").toString();
    } catch (err) {
   ...
   * @returns {Promise<void>}
   */
  async function publishCommentOnPR(
    commentContent,
    repo,
    pullRequestId,
    token
  ) {
    const apiUrl = `https://api.github.com/repos/${repo}/issues/${pullRequestId}/comments`;
    ...
    fetch(apiUrl, {
      ...
      .then((data) => {
        core.info("Comment published: " + data);
      })
      .catch((error) => {
    ...
    try {
      ...
      for (const comment of comments) {
        if (comment.body.includes(commentBodyInclude)) {
          await deleteComment(comment, repo, token);
        }
      }
    } catch (error) {
      core.setFailed(error.message);
  ...
  /**
   * Deletes the comment on the PR, matching comment id
   * @param comment
   * @param repo
   * @param token
   * @returns {Promise<void>}
   */
  async function deleteComment(comment, repo, token) {
    const response = await fetch(
  ...
  async function run() {
    try {
      // Regex pattern to match the major version
      const pattern = /^(\d+)\.\d+\.\d+$/;
      const currentVersion = core
        .getInput("last-release-version")
        .match(pattern)[1];
      core.info("Current version: " + currentVersion);
      const nextVersion = core
        .getInput("new-release-version")
        .match(pattern)[1];
      ...
      //Expecting the breaking change doc file path to be ${GITHUB_WORKSPACE}/docs/breaking-changes/v1.md
      const breakingChangeDocFilePath =
        core.getInput("breaking-change-doc-file-path") +
        "/v" +
        nextVersion +
        ".md";
      core.info("Breaking change doc file path: " + breakingChangeDocFilePath);

      // 1. Check for breaking changes
      const isBreakingChangeCommit = checkForBreakingChanges(
        currentVersion,
        nextVersion
      );

      if (!isBreakingChangeCommit) {
        core.info("It is not a breaking change");
        return;
      }

      // 2. Check if the breaking change doc exist
      const verifyFileExists = await checkFileExists(breakingChangeDocFilePath);

      if (verifyFileExists) {
        // 2.1. Find and delete all the breaking change template/doc comments on the PR
        // 2.2. Fetch the breaking change doc content
        // 2.3. Publish a new breaking change doc comment on the PR
        core.info("Breaking change doc exists");
        core.info(
        ...
        core.info("Reading breaking change doc file");
        const breakingChangeDocContent = readFile(breakingChangeDocFilePath);
        core.info("Publishing breaking change doc as comment on PR");
        await publishCommentOnPR(
          breakingChangeDocContent,
          repo,
          pullRequestId,
          githubToken
        );
        core.info("Breaking change doc published as comment on PR");
      } else {
        // 2.1. Find and delete previous breaking change template/doc comments on the PR
        // 2.2. Fetch the breaking change template content
        // 2.3. Publish the breaking change template comment on the PR
        // 2.4. Exit 1, set the error message
        core.info("Deleting previous breaking change template comment on PR");
        await deleteMatchingComments(
        ...
        const breakingChangeTemplateFilePath =
          process.env.GITHUB_ACTION_PATH + "/../templates/breaking-changes.md";
        core.info("Reading breaking change template doc file");
        const breakingChangeTemplateContent = readFile(
          breakingChangeTemplateFilePath
        );
        core.info("Publishing breaking change template doc as comment on PR");
        await publishCommentOnPR(
          breakingChangeTemplateContent,
          repo,
          pullRequestId,
          githubToken
        );
